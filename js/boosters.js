...
async function booster() {
    ...
    try {
        ...
        for (let i = 0; i < 5; i++) {
            ...
            pokemons.push({
                src: response.sprites.front_default, text: response.name, id: response.id, taille: response.height, poids: response.weight, type: response.types
            })
